...
npm run build --prefix "${PWD}/packages/lib-commons" --silent
#stop "$(color $GREEN)✓$(color)"

#start "" " $(color $CYAN)2.$(color) Building lib-api-client"
npm run build --prefix "${PWD}/packages/lib-api-client" --silent
#stop "$(color $GREEN)✓$(color)"

#start "" " $(color $CYAN)3.$(color) Building app-node-express"
npm run build --prefix "${PWD}/packages/app-node-express" --silent
#stop "$(color $GREEN)✓$(color)"

#echo -e "\n$(color $GREEN)✓ $(color $CYAN)4.$(color) Starting...\n"
export PACKAGE_JSON_VERSION=$(grep -o '\"version\": *\"[^\"]*\"' package.json | awk -F'\"' '{print $4}')
node --no-warnings --loader ts-node/esm --experimental-specifier-resolution=node "${PWD}/packages/app-node-express/dist/main.js"
