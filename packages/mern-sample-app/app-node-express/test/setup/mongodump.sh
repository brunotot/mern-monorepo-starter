...
$PWD_MONGODB_TOOLS_BIN/mongodump --uri=\"$DATABASE_URL/$DATABASE_NAME\" --out=$PWD_TEST_DUMP --authenticationDatabase admin --quiet
stop "$(color $GREEN)✓$(color)"

echo -e "\n$(color $GREEN)✓ $(color $CYAN)2.$(color) Making appropriate changes to the dump"
cd $PWD_APP_NODE_EXPRESS && mv $PWD_TEST_DUMP/$DATABASE_NAME/* $PWD_TEST_DUMP && rmdir $PWD_TEST_DUMP/$DATABASE_NAME
