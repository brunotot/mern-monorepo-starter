...
import type { MongoClient } from "@org/lib-mongodb";
import type { IncomingMessage, Server, ServerResponse } from "http";
import { getTypedError } from "@org/lib-api-client";
import { MongoDatabaseService } from "@org/lib-mongodb";
import express from "express";
import { initializeExpressRoutes, initializeSwagger, TsRestRouterService } from "@/lib/@ts-rest";
import { IocRegistry } from "@/lib/ioc";
import { log, logBanner } from "@/lib/winston";
...
export class ExpressApp {
  public static readonly instance: ExpressApp;
  ...
  public async init(mocks: Record<string, NoArgsClass> = {}): Promise<void> {
    const nonTestMode = env.SERVER_ENV !== "test";
    nonTestMode && log.info("Initializing Swagger");
    this.#initializeSwagger();
    nonTestMode && log.info("Initializing IoC container");
    this.#initializeIoc(mocks);
    nonTestMode && log.info(`Initializing global middleware (${this.middleware.length})`);
    this.#initializeGlobalMiddlewares();
    nonTestMode &&
      log.info(`Initializing routes (${TsRestRouterService.getInstance().getTotalRouteCount()})`);
    this.#initializeExpressRoutes();
    nonTestMode && log.info("Initializing global error handler");
    this.#initializeErrorHandlerMiddleware();
    nonTestMode && log.info("Connecting to database");
    await this.#initializeDatabase();
    nonTestMode && log.info("App successfully initialized!");
  }
